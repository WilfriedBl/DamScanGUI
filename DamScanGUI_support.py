...
# 0.1.6 New tags for GPS tolerance (DamScan/DamCompare 1.5.0)
...
def set_Tk_var():
    ...
    global txt_dist_tolerance; txt_dist_tolerance = StringVar()
    global txt_alt_tolerance; txt_alt_tolerance = StringVar()
...
def read_ini(args, conf):
    ...
    if args.user is None:           args.user = conf.get('Database', 'User', fallback="postgres/postgres")
    if args.fullpath is None:       args.fullpath = conf.getboolean('Session', 'Fullpath', fallback=False)
    if args.dist_tolerance is None: args.dist_tolerance = conf.getfloat('Session', 'GPS_dist', fallback=0.0)
    if args.alt_tolerance is None:  args.alt_tolerance = conf.getfloat('Session', 'GPS_alt', fallback=0.0)
...
def populate_GUI_panel():  # Set Values to display at the GUI
    ...
    txt_dist_tolerance.set(args.dist_tolerance) # --GPS_dist        Allowed GPS distance tolerance
    txt_alt_tolerance.set(args.alt_tolerance)   # --GPS_alt         Allowed GPS height tolerance
...
def WriteIni(damscaninifile):
    ...
    if args.dbname is not None: conf['Database']['Catalog'] = str(args.dbname)
    elif args.sqlite:           conf['Database']['Catalog'] = "DaminionCatalog.dmc"
    else:                       conf['Database']['Catalog'] = "NetCatalog"

    if args.server is not None and not args.sqlite: conf['Database']['Server'] = str(args.server).rstrip(" ")
    if args.port   is not None and not args.sqlite: conf['Database']['Port'] = str(args.port)
    if args.user   is not None and not args.sqlite: conf['Database']['User'] = str(args.user)

    conf['Session'] = {}
    if args.fullpath:           conf['Session']['Fullpath'] = "True"
    if args.id:                 conf['Session']['ID'] = "True"
    if args.dist_tolerance:     conf['Session']['GPS_dist'] = args.dist_tolerance
    if args.alt_tolerance:      conf['Session']['GPS_alt'] =  args.alt_tolerance

    if args.basename not in (None, "", " "):conf['Session']['Basename'] = str(args.basename).strip("(,')")
    elif ckb_verify_basenames.get():        conf['Session']['Basename'] = ""
    else:                                   conf.remove_option('Session', 'Basename')

    if args.group:                          conf['Session']['Group'] = "True"
    if args.taglist is None:                conf['Session']['Tags'] = "All"
    else:                                   conf['Session']['Tags'] = ' '.join(args.taglist)
    if args.exfile != None:
        if args.exfile != "":               conf['Session']['Exclude'] = str(args.exfile)
    if args.exdir != None:
        if args.exdir != "":                conf['Session']['ExcludePaths'] = str(args.exdir)
    if args.onlydir != None:
        if args.onlydir != "":              conf['Session']['OnlyPaths'] = str(args.onlydir)
    if args.onlyfile != None:
        if args.onlyfile != "":             conf['Session']['Only'] = str(args.onlyfile)
    if args.ack_pairs != None:
        if args.ack_pairs != "":            conf['Session']['AckPairs'] = str(args.ack_pairs)
    if args.outfilename != None:
        if args.outfilename != "<stdout>":  conf['Session']['Outfile'] = str(args.outfilename)
    if args.verbose == 1:                   conf['Session']['Verbose'] = '1'
    elif args.verbose == 2:                 conf['Session']['Verbose'] = '2'
    else:                                   conf['Session']['Verbose'] = '0'
...
def save_options_to_ini_files(event=None):
    ...
    if analysis_type == 'Scan': show_cmd_str_scan()
    else:                       show_cmd_str_compare()
...
def build_CMD_line():
    ...
    if txt_dist_tolerance.get() not in ('0.0', ''):
        args.dist_tolerance = txt_dist_tolerance.get()
        newArgString += ' --GPS_dist '+args.dist_tolerance
    if txt_alt_tolerance.get() != '0.0':
        args.alt_tolerance = txt_alt_tolerance.get()
        newArgString += ' --GPS_alt '+args.alt_tolerance
